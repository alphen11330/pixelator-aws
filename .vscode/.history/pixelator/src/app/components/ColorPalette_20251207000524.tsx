...
const hexToRgb = (hex: string): string => {
  // 1. # を除去
  const standardHex = hex.startsWith("#") ? hex.slice(1) : hex;

  if (standardHex.length !== 6) return "rgb(0,0,0)";

  // 2. 各成分を16進数から数値に変換
  const r = parseInt(standardHex.substring(0, 2), 16);
  const g = parseInt(standardHex.substring(2, 4), 16);
  const b = parseInt(standardHex.substring(4, 6), 16);

  // 3. 🚨 修正箇所: 数値の配列ではなく、RGB文字列を返す
  return `rgb(${r},${g},${b})`; // <= 戻り値の型が string になる
};
        ...
        {colorPalette.map((color, index) => (
          ...
          >
            <input
              ...
              title={color}
              onChange={(e) => {
                // 1. input から受け取った値 (常にHEXコード)
                const newHexValue = e.target.value;

                // 2. 🚨 実行箇所: HEXコードを直ちに RGB文字列に変換
                const newRgbString = hexToRgb(newHexValue);

                // 3. 変換後のRGB文字列をステート更新関数に渡す
                handleColorChange(index, newRgbString);
              }}
              style={{
                position: "absolute",
                top: 0,
