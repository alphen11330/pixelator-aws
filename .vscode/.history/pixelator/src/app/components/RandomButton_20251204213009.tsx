...
const RandomButton: React.FC<Props> = ({
  ...
  const setRandomNatural = () => {
    ...
    // Contrast Level: 1.0～1.2 (0.1刻み)
    const randomContrastLevel =
      Math.floor(Math.random() * ((1.2 - 1.0) / 0.1 + 1)) * 0.1 + 1.0;
    setContrastLevel(randomContrastLevel);

    // Hue: 0～179 (1刻み)
    // 0または1をランダムに選ぶ (0なら前半の範囲、1なら後半の範囲)
    const rangeSelector = Math.floor(Math.random() * 2);
    let randomHue;
    if (rangeSelector === 0) {
      // 前半の範囲: 0～9 (10種類の値)
      randomHue = Math.floor(Math.random() * 10); // 0～9
    } else {
      // 後半の範囲: 170～179 (10種類の値)
      // 0～9の乱数を生成し、170を足す
      randomHue = 170 + Math.floor(Math.random() * 10); // 170～179
    }
    setHue(randomHue);
    ...
    <>
      ...
      >
        ...
        <button
          ...
          className={style.purpleShineButton}
